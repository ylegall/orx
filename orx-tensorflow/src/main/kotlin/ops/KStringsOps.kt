...
interface KStringsOps {
    ...
    fun join(inputs : Iterable<Operand<TString>>, options : Join.Options) : Output<TString> {
        val op = stringsOps.join(inputs, options)
        return op.asOutput()
    }

    fun lower(input : Operand<TString>, options : Lower.Options) : Output<TString> {
        val op = stringsOps.lower(input, options)
        return op.asOutput()
    }

    fun reduceJoin(inputs : Operand<TString>, reductionIndices : Operand<TInt32>, options : ReduceJoin.Options) : Output<TString> {
    ...
    fun regexReplace(input : Operand<TString>, pattern : Operand<TString>, rewrite : Operand<TString>, options : RegexReplace.Options) : Output<TString> {
        val op = stringsOps.regexReplace(input, pattern, rewrite, options)
        return op.asOutput()
    }

    fun stringFormat(inputs : Iterable<Operand<*>>, options : StringFormat.Options) : Output<TString> {
        val op = stringsOps.stringFormat(inputs, options)
        return op.asOutput()
    }

    fun stringLength(input : Operand<TString>, options : StringLength.Options) : Output<TInt32> {
    ...
    fun <T : TNumber> substr(input : Operand<TString>, pos : Operand<T>, len : Operand<T>, options : Substr.Options) : Output<TString> {
    ...
    fun unicodeTranscode(input : Operand<TString>, inputEncoding : String, outputEncoding : String, options : UnicodeTranscode.Options) : Output<TString> {
        val op = stringsOps.unicodeTranscode(input, inputEncoding, outputEncoding, options)
        return op.asOutput()
    }

    fun <T : TNumber, U : TNumber> unsortedSegmentJoin(inputs : Operand<TString>, segmentIds : Operand<T>, numSegments : Operand<U>, options : UnsortedSegmentJoin.Options) : Output<TString> {
        val op = stringsOps.unsortedSegmentJoin(inputs, segmentIds, numSegments, options)
        return op.asOutput()
    }

    fun upper(input : Operand<TString>, options : Upper.Options) : Output<TString> {
