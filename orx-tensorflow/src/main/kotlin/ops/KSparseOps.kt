...
interface KSparseOps {
    ...
    fun <T : TType> addManySparseToTensorsMap(sparseIndices : Operand<TInt64>, sparseValues : Operand<T>, sparseShape : Operand<TInt64>, options : AddManySparseToTensorsMap.Options) : Output<TInt64> {
        val op = sparseOps.addManySparseToTensorsMap(sparseIndices, sparseValues, sparseShape, options)
        return op.asOutput()
    }

    fun <T : TType> addSparseToTensorsMap(sparseIndices : Operand<TInt64>, sparseValues : Operand<T>, sparseShape : Operand<TInt64>, options : AddSparseToTensorsMap.Options) : Output<TInt64> {
        val op = sparseOps.addSparseToTensorsMap(sparseIndices, sparseValues, sparseShape, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> sparseBincount(indices : Operand<TInt64>, values : Operand<T>, denseShape : Operand<TInt64>, size : Operand<T>, weights : Operand<U>, options : SparseBincount.Options) : Output<U> {
        val op = sparseOps.sparseBincount(indices, values, denseShape, size, weights, options)
        return op.asOutput()
    }

    fun <T : TType> sparseConditionalAccumulator(dtype : DataType<T>, shape : Shape, options : SparseConditionalAccumulator.Options) : Output<TString> {
    ...
    fun <T : TNumber, U : TNumber> sparseMatMul(a : Operand<T>, b : Operand<U>, options : SparseMatMul.Options) : Output<TFloat32> {
        val op = sparseOps.sparseMatMul(a, b, options)
        return op.asOutput()
    }

    fun <T : TNumber> sparseReduceMax(inputIndices : Operand<TInt64>, inputValues : Operand<T>, inputShape : Operand<TInt64>, reductionAxes : Operand<TInt32>, options : SparseReduceMax.Options) : Output<T> {
        val op = sparseOps.sparseReduceMax(inputIndices, inputValues, inputShape, reductionAxes, options)
        return op.asOutput()
    }

    fun <T : TType> sparseReduceSum(inputIndices : Operand<TInt64>, inputValues : Operand<T>, inputShape : Operand<TInt64>, reductionAxes : Operand<TInt32>, options : SparseReduceSum.Options) : Output<T> {
    ...
    fun <U : TType, T : TNumber> sparseTensorDenseMatMul(aIndices : Operand<T>, aValues : Operand<U>, aShape : Operand<TInt64>, b : Operand<U>, options : SparseTensorDenseMatMul.Options) : Output<U> {
        val op = sparseOps.sparseTensorDenseMatMul(aIndices, aValues, aShape, b, options)
        return op.asOutput()
    }

    fun <U : TType, T : TNumber> sparseToDense(sparseIndices : Operand<T>, outputShape : Operand<T>, sparseValues : Operand<U>, defaultValue : Operand<U>, options : SparseToDense.Options) : Output<U> {
