...
interface KImageOps {
    ...
    fun <T : TNumber> cropAndResize(image: Operand<T>, boxes: Operand<TFloat32>, boxInd: Operand<TInt32>, cropSize: Operand<TInt32>, vararg options: CropAndResize.Options): Output<TFloat32> {
        val op = imageOps.cropAndResize(image, boxes, boxInd, cropSize, *options)
        return op.asOutput()
    }

    fun <T : TNumber> cropAndResizeGradBoxes(grads: Operand<TFloat32>, image: Operand<T>, boxes: Operand<TFloat32>, boxInd: Operand<TInt32>, vararg options: CropAndResizeGradBoxes.Options): Output<TFloat32> {
        val op = imageOps.cropAndResizeGradBoxes(grads, image, boxes, boxInd, *options)
        return op.asOutput()
    }

    fun <T : TNumber> cropAndResizeGradImage(grads: Operand<TFloat32>, boxes: Operand<TFloat32>, boxInd: Operand<TInt32>, imageSize: Operand<TInt32>, t: DataType<T>, vararg options: CropAndResizeGradImage.Options): Output<T> {
        val op = imageOps.cropAndResizeGradImage(grads, boxes, boxInd, imageSize, t, *options)
        return op.asOutput()
    }

    fun decodeAndCropJpeg(contents: Operand<TString>, cropWindow: Operand<TInt32>, vararg options: DecodeAndCropJpeg.Options): Output<TUint8> {
        val op = imageOps.decodeAndCropJpeg(contents, cropWindow, *options)
        return op.asOutput()
    }

    fun decodeBmp(contents: Operand<TString>, vararg options: DecodeBmp.Options): Output<TUint8> {
    ...
    fun decodeJpeg(contents: Operand<TString>, vararg options: DecodeJpeg.Options): Output<TUint8> {
        val op = imageOps.decodeJpeg(contents, *options)
        return op.asOutput()
    }

    fun decodePng(contents: Operand<TString>, vararg options: DecodePng.Options): Output<TUint8> {
        val op = imageOps.decodePng(contents, *options)
        return op.asOutput()
    }

    fun <T : TNumber> decodePng(contents: Operand<TString>, dtype: DataType<T>, vararg options: DecodePng.Options): Output<T> {
    ...
    fun encodeJpeg(image: Operand<TUint8>, vararg options: EncodeJpeg.Options): Output<TString> {
    ...
    fun <T : TNumber> encodePng(image: Operand<T>, vararg options: EncodePng.Options): Output<TString> {
    ...
    fun <T : TNumber> randomCrop(image: Operand<T>, size: Operand<TInt64>, vararg options: RandomCrop.Options): Output<T> {
        val op = imageOps.randomCrop(image, size, *options)
        return op.asOutput()
    }

    fun <T : TNumber> resizeArea(images: Operand<T>, size: Operand<TInt32>, vararg options: ResizeArea.Options): Output<TFloat32> {
        val op = imageOps.resizeArea(images, size, *options)
        return op.asOutput()
    }

    fun <T : TNumber> resizeBicubic(images: Operand<T>, size: Operand<TInt32>, vararg options: ResizeBicubic.Options): Output<TFloat32> {
        val op = imageOps.resizeBicubic(images, size, *options)
        return op.asOutput()
    }

    fun <T : TNumber> resizeBilinear(images: Operand<T>, size: Operand<TInt32>, vararg options: ResizeBilinear.Options): Output<TFloat32> {
        val op = imageOps.resizeBilinear(images, size, *options)
        return op.asOutput()
    }

    fun <T : TNumber> resizeNearestNeighbor(images: Operand<T>, size: Operand<TInt32>, vararg options: ResizeNearestNeighbor.Options): Output<T> {
    ...
    fun <T : TNumber> scaleAndTranslate(images: Operand<T>, size: Operand<TInt32>, scale: Operand<TFloat32>, translation: Operand<TFloat32>, vararg options: ScaleAndTranslate.Options): Output<TFloat32> {
