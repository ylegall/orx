...
interface KRandomOps {
    ...
    fun <T : TNumber> multinomial(logits : Operand<T>, numSamples : Operand<TInt32>, options : Multinomial.Options) : Output<TInt64> {
        val op = randomOps.multinomial(logits, numSamples, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> multinomial(logits : Operand<T>, numSamples : Operand<TInt32>, outputDtype : DataType<U>, options : Multinomial.Options) : Output<U> {
        val op = randomOps.multinomial(logits, numSamples, outputDtype, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> parameterizedTruncatedNormal(shape : Operand<T>, means : Operand<U>, stdevs : Operand<U>, minvals : Operand<U>, maxvals : Operand<U>, options : ParameterizedTruncatedNormal.Options) : Output<U> {
        val op = randomOps.parameterizedTruncatedNormal(shape, means, stdevs, minvals, maxvals, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> randomGamma(shape : Operand<T>, alpha : Operand<U>, options : RandomGamma.Options) : Output<U> {
        val op = randomOps.randomGamma(shape, alpha, options)
        return op.asOutput()
    }

    fun <T : TNumber, U : TNumber> randomPoisson(shape : Operand<T>, rate : Operand<U>, options : RandomPoisson.Options) : Output<TInt64> {
        val op = randomOps.randomPoisson(shape, rate, options)
        return op.asOutput()
    }

    fun <V : TNumber, T : TNumber, U : TNumber> randomPoisson(shape : Operand<T>, rate : Operand<U>, dtype : DataType<V>, options : RandomPoisson.Options) : Output<V> {
        val op = randomOps.randomPoisson(shape, rate, dtype, options)
        return op.asOutput()
    }

    fun <T : TType> randomShuffle(value : Operand<T>, options : RandomShuffle.Options) : Output<T> {
        val op = randomOps.randomShuffle(value, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> randomStandardNormal(shape : Operand<T>, dtype : DataType<U>, options : RandomStandardNormal.Options) : Output<U> {
        val op = randomOps.randomStandardNormal(shape, dtype, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> randomUniform(shape : Operand<T>, dtype : DataType<U>, options : RandomUniform.Options) : Output<U> {
        val op = randomOps.randomUniform(shape, dtype, options)
        return op.asOutput()
    }

    fun <U : TNumber, T : TNumber> randomUniformInt(shape : Operand<T>, minval : Operand<U>, maxval : Operand<U>, options : RandomUniformInt.Options) : Output<U> {
        val op = randomOps.randomUniformInt(shape, minval, maxval, options)
        return op.asOutput()
    }

    fun recordInput(filePattern : String, options : RecordInput.Options) : Output<TString> {
    ...
    fun <U : TNumber, T : TNumber> truncatedNormal(shape : Operand<T>, dtype : DataType<U>, options : TruncatedNormal.Options) : Output<U> {
