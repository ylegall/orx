...
interface KLinalgOps {
    ...
    fun <T : TNumber> batchMatrixInverse(input : Operand<T>, options : BatchMatrixInverse.Options) : Output<T> {
    ...
    fun <T : TNumber> batchMatrixSolve(matrix : Operand<T>, rhs : Operand<T>, options : BatchMatrixSolve.Options) : Output<T> {
        val op = linalgOps.batchMatrixSolve(matrix, rhs, options)
        return op.asOutput()
    }

    fun <T : TNumber> batchMatrixSolveLs(matrix : Operand<T>, rhs : Operand<T>, l2Regularizer : Operand<TFloat64>, options : BatchMatrixSolveLs.Options) : Output<T> {
        val op = linalgOps.batchMatrixSolveLs(matrix, rhs, l2Regularizer, options)
        return op.asOutput()
    }

    fun <T : TNumber> batchMatrixTriangularSolve(matrix : Operand<T>, rhs : Operand<T>, options : BatchMatrixTriangularSolve.Options) : Output<T> {
    ...
    fun <T : TType> inv(input : Operand<T>, options : Inv.Options) : Output<T> {
        val op = linalgOps.inv(input, options)
        return op.asOutput()
    }

    fun loadAndRemapMatrix(ckptPath : Operand<TString>, oldTensorName : Operand<TString>, rowRemapping : Operand<TInt64>, colRemapping : Operand<TInt64>, initializingValues : Operand<TFloat32>, numRows : Long, numCols : Long, options : LoadAndRemapMatrix.Options) : Output<TFloat32> {
    ...
    fun <T : TType> matMul(a : Operand<T>, b : Operand<T>, options : MatMul.Options) : Output<T> {
    ...
    fun <T : TType> matrixDiagPartV3(input : Operand<T>, k : Operand<TInt32>, paddingValue : Operand<T>, options : MatrixDiagPartV3.Options) : Output<T> {
        val op = linalgOps.matrixDiagPartV3(input, k, paddingValue, options)
        return op.asOutput()
    }

    fun <T : TType> matrixDiagV3(diagonal : Operand<T>, k : Operand<TInt32>, numRows : Operand<TInt32>, numCols : Operand<TInt32>, paddingValue : Operand<T>, options : MatrixDiagV3.Options) : Output<T> {
        val op = linalgOps.matrixDiagV3(diagonal, k, numRows, numCols, paddingValue, options)
        return op.asOutput()
    }

    fun <T : TType> matrixSetDiag(input : Operand<T>, diagonal : Operand<T>, k : Operand<TInt32>, options : MatrixSetDiag.Options) : Output<T> {
        val op = linalgOps.matrixSetDiag(input, diagonal, k, options)
        return op.asOutput()
    }

    fun <T : TType> matrixSolveLs(matrix : Operand<T>, rhs : Operand<T>, l2Regularizer : Operand<TFloat64>, options : MatrixSolveLs.Options) : Output<T> {
        val op = linalgOps.matrixSolveLs(matrix, rhs, l2Regularizer, options)
        return op.asOutput()
    }

    fun <T : TType> solve(matrix : Operand<T>, rhs : Operand<T>, options : Solve.Options) : Output<T> {
    ...
    fun <T : TType> triangularSolve(matrix : Operand<T>, rhs : Operand<T>, options : TriangularSolve.Options) : Output<T> {
